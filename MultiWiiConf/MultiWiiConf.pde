...
//Added For  Processing 2.x compabillity
import java.util.*;
import java.io.*;
//****************************
...
cGraph g_graph;
int windowsX    = 1200;       int windowsY    = 900;
...
int arduType; //1 for quadX , 2 for Hex6X
...
static int RCThro = 3, RCRoll = 0, RCPitch =1, RCYaw =2, RCAUX1=4, RCAUX2=5, RCAUX3=6, RCAUX4=7; //in TX_StickSlider[] 
...
Toggle TX_StickToggle[] = new Toggle[8];
...
void setup() {
  ...
  ControlFont fontBtn_12 = new ControlFont(font12,241);
  addTabs();  
...
//B area
  ...
  controlP5.addTextlabel("accrolllabel","   ROLL",xo,y1+10).setFont(font12);
  controlP5.addTextlabel("accpitchlabel","   PITCH",xo,y1+20).setFont(font12);
  controlP5.addTextlabel("acczlabel","   Z",xo,y1+30).setFont(font12);
  controlP5.addTextlabel("gyrolabel","GYRO",xo,y2 -4).setFont(font12);
  controlP5.addTextlabel("gyrorolllabel","   ROLL",xo,y2+10).setFont(font12);
  controlP5.addTextlabel("gyropitchlabel","   PITCH",xo,y2+20).setFont(font12);
  controlP5.addTextlabel("gyroyawlabel","   YAW",xo,y2+30).setFont(font12);
  controlP5.addTextlabel("maglabel","MAG",xo,y5 -4).setFont(font12);
  controlP5.addTextlabel("magrolllabel","   ROLL",xo,y5+10).setFont(font12);
  controlP5.addTextlabel("magpitchlabel","   PITCH",xo,y5+20).setFont(font12);
  controlP5.addTextlabel("magyawlabel","   YAW",xo,y5+30).setFont(font12);
  ...
  controlP5.addTextlabel("debug1","debug1",x+90,y6 -2).setFont(font12);
  controlP5.addTextlabel("debug2","debug2",x+210,y6 -2).setFont(font12);
  controlP5.addTextlabel("debug3","debug3",x+330,y6 -2).setFont(font12);
  controlP5.addTextlabel("debug4","debug4",x+450,y6 -2).setFont(font12);
...
//B area - End
  for( i=0;i<PIDITEMS;i++) {//ROLL,PITCH,YAW,ALT -> confP[0] ~ confP[9] & confI[0] ~ confI[9] & confD[0] ~ confD[9] "P" "I" "D"
    ...
    confD[i] = (controlP5.Numberbox) hideLabel(controlP5.addNumberbox("confD"+i,0,xParam+120,yParam+20+i*17,40,14));
  ...
  confI[8].hide();confD[8].hide();confD[4].hide();//change bounds for MAG-9 "I" & "D" is hide ; change bounds for Pos-4 "D" is hide 
  confP[9].hide();confI[9].hide();confD[9].hide();//confP[9],confI[9],and confD[9] is no exist, so hide.
  //change bounds for POS-4 
  confP[4].setDecimalPrecision(2).setMultiplier(0.01).setMax(5);
  confI[4].setDecimalPrecision(1).setMultiplier(0.1) .setMax(2.5);
  //change bounds for POSR-5
  ...
  buttonWRITE =         controlP5.addButton("WRITE", 1, xParam+300, yParam+260, 60, 16)  .setColorBackground(red_);
  ...
  buttonCALIBRATE_ACC = controlP5.addButton("CALIB_ACC",  1,  xParam+210, yParam+260, 75, 16).setColorBackground(red_);
  buttonCALIBRATE_MAG = controlP5.addButton("CALIB_MAG",  1,  xParam+130, yParam+260, 75, 16).setColorBackground(red_);
  ...
  btMagDecl    =        controlP5.addButton("MagDecl"  ,  1,  xParam+350, yParam+100, 180, 16).setColorBackground(red_).moveTo("Config").setLabel("magnetic-declination.com");
  btMWiiHome   =        controlP5.addButton("MWiiHome" ,  1,  xParam+350, yParam+60, 180, 16).setColorBackground(red_).moveTo("Config").setLabel("MUltiWii Forum");
  btDownloads  =        controlP5.addButton("MWiiGet"  ,  1,  xParam+350, yParam+80, 180, 16).setColorBackground(red_).moveTo("Config").setLabel("MUltiWii Downloads");
  
//Mode 2 Controler
// Sliders for Transmitter - Width:10 , Height:100
  TX_StickSlider[RCThro ] =  controlP5.addSlider("Throt",900,2100,1500,xRC+5,yRC+10,10,100).setDecimalPrecision(0);//left
  TX_StickSlider[RCPitch] =  controlP5.addSlider("Pitch",900,2100,1500,xRC+90,yRC+10,10,100).setDecimalPrecision(0);//right
// Sliders for Transmitter - Width:100 , Height:10
  TX_StickSlider[RCYaw ]  =  controlP5.addSlider("Yaw",  900,2100,1500,xRC,yRC+170,100,10).setDecimalPrecision(0);
  TX_StickSlider[RCRoll ] =  controlP5.addSlider("Roll", 900,2100,1500,xRC,yRC+185,100,10).setDecimalPrecision(0);
  TX_StickSlider[RCAUX1]  =  controlP5.addSlider("AUX1", 1000,2000,1500,xRC,yRC+200,100,10) .setDecimalPrecision(0);
  TX_StickSlider[RCAUX2]  =  controlP5.addSlider("AUX2", 1000,2000,1500,xRC,yRC+215,100,10) .setDecimalPrecision(0);

  motorControlSlider      = controlP5.addSlider("Motors",1000,2000,1500,xParam+410, yParam,90,30)   .setDecimalPrecision(0).hide();

  for( i=0;i<8;i++) {
    motToggle[i]     = controlP5.addToggle("M"+i,false,xMot-100,yMot+115,20,15).moveTo("Motors");//.hide();
...
//DON'T TO CHANGE -- end
  ...
  // version
  fill(255, 255, 255);
  text("k998136.pixnet.net",16,19+tabHeight);
  text("Version",16,35+tabHeight);
  ...
  text("dis Home :",xGPS,yGPS+90);
  // ---------------------------------------------------------------------------------------------

  text("I2C error:",xGraph+350,yGraph-10);
  text("Cycle Time:",xGraph+220,yGraph-10);
  rect(xGraph - 5 , yGraph + 220 , xGraph + 250 , yGraph + 320 );
  fill(0, 0, 0);
  text("Battery", xGraph +5 , yGraph + 240);
  text("   Total    :", xGraph + tabHeight , yGraph + 260);
  text(pMeterSum * 1.0 + "  mAh", xGraph + 150, yGraph + 260 );

  text("   Current  :", xGraph + tabHeight , yGraph + 280);
  text(amperage/10.0 + "  A", xGraph + 150, yGraph + 280 );
    
  text("   Voltage  :", xGraph + tabHeight , yGraph + 300);
  text(bytevbat/10.0 + "  V", xGraph + 150, yGraph + 300 );
  ...
  int MotToggleMove=200;
   ...
   if (multiType == QUADX) { //QUAD X
    ...
    noLights();text("QUAD X", -30,-50);camera();popMatrix();
    
    textFont(font15);
    text("Motor",xMot+50,yMot+280);
    motSlider[0].setPosition(xMot+90,yMot+230).setHeight(60).setCaptionLabel("9") .show();
    motSlider[1].setPosition(xMot+90,yMot+150).setHeight(60).setCaptionLabel("10").show();
    motSlider[2].setPosition(xMot+10,yMot+230).setHeight(60).setCaptionLabel("11") .show();
    motSlider[3].setPosition(xMot+10,yMot+150).setHeight(60).setCaptionLabel("3").show(); 
    ...
    noLights();camera();popMatrix();  //no anything
  }
  ...
  // rc background
  rect(xRC-5,yRC-5, xRC+145, yRC+280);
  // motor background
  rect(xMot-5,yMot+145, xMot+145, yMot+350);
...
void drawMotor(float x1, float y1, int mot_num, char dir) {   //Code by Danal
  ...
  ellipse(x1, y1, size, size);//circle size
  textFont(font15);//circle text
  textAlign(CENTER);//circle text center
...
void InitSerial(float portValue) {
  if (portValue < commListMax) {
    ...
    graphEnabled = true;//draw graph start
    g_serial.buffer(256);
    btnQConnect.hide();
  } else {
    ...
    graphEnabled = false;//draw graph stop
    init_com=0;
    g_serial.stop();
    btnQConnect.show();
  }
